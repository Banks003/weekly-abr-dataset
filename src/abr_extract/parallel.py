...
def build_sqlite_from_parquet(
    ...
    *,
    ...
    Reads each of the three merged Parquets in `batch_size` row chunks and
    streams them through `sqlite3.executemany`. Schema and index shape match
    what the sequential `WriterBundle` produced. Used after the parallel
    parse + merge step.
    """
    # Imports kept local so this module stays cheap to import for code
    # paths that don't need SQLite materialisation (e.g. tests focused on
    # parquet-only behaviour).
    import sqlite3

    from .schema import SQLITE_DDL
    from .write import _main_row_tuple, _sqlite_main_insert, _sqlite_value
    ...
    try:
        ...
        for batch in pq.ParquetFile(main_parquet).iter_batches(batch_size=batch_size):
            rows = batch.to_pylist()
            if not rows:
                continue
            conn.executemany(
                _sqlite_main_insert(),
                [_main_row_tuple(r) for r in rows],
            )
            counts.main += len(rows)
        conn.commit()

        for batch in pq.ParquetFile(trading_parquet).iter_batches(batch_size=batch_size):
            rows = batch.to_pylist()
            if not rows:
                continue
            conn.executemany(
                "INSERT INTO abn_trading_names (abn, name, name_type) VALUES (?, ?, ?)",
                [(r["abn"], r["name"], r["name_type"]) for r in rows],
            )
            counts.trading += len(rows)
        conn.commit()

        for batch in pq.ParquetFile(dgr_parquet).iter_batches(batch_size=batch_size):
            rows = batch.to_pylist()
            if not rows:
                continue
            conn.executemany(
